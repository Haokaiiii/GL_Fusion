...
def evaluate_validation_set(model, test_sequences, task_df, indices, node_mapping, device, x_scaler, y_scaler):
    ...
    # Apply fast evaluation limits from environment variables
    max_val_users = int(os.environ.get('MAX_VAL_USERS', '1000'))
    eval_subsample_ratio = float(os.environ.get('EVAL_SUBSAMPLE_RATIO', '1.0'))
    
    # Apply subsampling
    if eval_subsample_ratio < 1.0:
        target_users = int(len(all_val_users) * eval_subsample_ratio)
        target_users = min(target_users, max_val_users)
        if target_users < len(all_val_users):
            import random
            random.seed(42)  # For reproducibility
            val_users = set(random.sample(list(all_val_users), target_users))
            logger.info(f'Fast evaluation: Using {len(val_users)} users out of {len(all_val_users)} total ({eval_subsample_ratio*100:.1f}%)')
        else:
            val_users = all_val_users
    else:
        # Still limit by max_val_users even if no subsampling
        if len(all_val_users) > max_val_users:
            import random
            random.seed(42)
